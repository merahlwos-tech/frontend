...
function ConfirmationPage() {
  ...
  const orderId = location.state?.orderId || sessionStorage.getItem('cancelOrderId')
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      setProgress(pct)
      if (pct <= 0) { clearInterval(intervalRef.current); if (!cancelledRef.current) setShowPopup(false) }
    }, 50)
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', background: '#F9F8FC' }}>
      <div style={{ width: '100%', maxWidth: 400, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 10, fontWeight: 800, color: '#4A8C6A', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 10 }}>{t('conf_badge')}</p>
        <h1 style={{ fontFamily: 'Dancing Script, cursive', fontSize: '2.8rem', fontWeight: 700, color: '#1A1A2E', lineHeight: 1.05, marginBottom: 14 }}>{t('conf_title')}</h1>
        <p style={{ fontSize: 14, color: '#666', lineHeight: 1.8, marginBottom: 28 }}>{t('conf_body')}</p>
        <div style={{ background: 'white', borderRadius: 16, padding: '16px 20px', marginBottom: 24, border: '1px solid #F0EDF5', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#1A1A2E', marginBottom: 3 }}>{t('conf_delivery')}</p>
            <p style={{ fontSize: 12, color: '#888' }}>{t('conf_delivery_sub')}</p>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <Link to="/products" style={{ background: '#1A1A2E', color: 'white', borderRadius: 50, padding: '13px 28px', textDecoration: 'none', fontSize: 14, fontWeight: 700, fontFamily: 'Nunito, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            <ShoppingBag size={15} /> {t('conf_shop')}
          </Link>
          <Link to="/" style={{ background: 'white', color: '#555', borderRadius: 50, padding: '12px 28px', textDecoration: 'none', fontSize: 14, fontWeight: 600, fontFamily: 'Nunito, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, border: '1px solid #E8E5F0' }}>
            <Home size={15} /> {t('conf_home')}
          </Link>
        </div>
      </div>

      {showPopup && orderId && (
        <div style={{ position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 9999, width: 340, maxWidth: 'calc(100vw - 32px)', background: 'white', borderRadius: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.15)', border: '1px solid #F0EDF5', overflow: 'hidden', animation: 'slideDown 0.3s ease' }}>
          <div style={{ padding: '14px 16px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#F0F7EE', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <CheckCircle size={15} style={{ color: '#4A8C6A' }} />
              </div>
              <div>
                <p style={{ fontSize: 13, fontWeight: 800, color: '#1A1A2E' }}>{t('cancel_title')}</p>
                <p style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{t('cancel_sub')}</p>
              </div>
            </div>
      ...
      <style>{`@keyframes slideDown{from{transform:translateX(-50%) translateY(-12px);opacity:0}to{transform:translateX(-50%) translateY(0);opacity:1}}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
    </div>
  )
}
